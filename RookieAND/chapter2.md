...
- `docker attach`: Detached 모드로 실행된 컨테이너의 로그를 확인할 수 있습니다.
...
- 컨테이너 생성 시 기본적으로는 `docker0` 브맿지를 통해 외부와 통신 가능한 환경을 사용할 수 있습니다.
- 하지만 사용자의 선택에 따라 여러 네트워크 드라이버를 사용할 수도 있습니다.

2. 지원되는 드라이버

- 도커 자체 제공 네트워크 드라이버: bridge, host, none, container, overlay
- 서드 파티 플러그인 솔루션: weave, flannel, openvswitch 등
- `docker network ls` 명령어로 네트워크 목록을 확인할 수 있습니다. (이미 bridge, host, none은 존재함)
